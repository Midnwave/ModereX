...
/**
 * /ipban <player> [<duration | reason>]{reason} [flags]
 * Aliases: /banip, /ban-ip
 *
 * Bans a player by their account and IP address.
 * Player must be online to obtain IP address.
 */
public class IPBanCommand extends PunishmentCommandBase {

    public IPBanCommand(ModereX plugin) {
        super(plugin, "moderex.ipban", false);
    }
    ...
    protected void execute(CommandSender sender, String[] args) {
        ...
        if (regularArgs.isEmpty()) {
            sendMessage(sender, "<red>Usage: /ipban <player> [duration] [reason] [flags]");
            return;
        }
        ...
        executeIPBan(context, target, ipAddress);
    }

    private void executeIPBan(PunishmentContext context, Player target, String ipAddress) {
    ...
    private void broadcastPunishment(PunishmentContext context, String playerName, String duration, String reason) {
        ...
        if (context.isExtraSilent()) {
            plugin.getLogger().info(String.format("%s IP banned %s for %s. Reason: %s",
        ...
        org.bukkit.Bukkit.getScheduler().runTask(plugin, () -> {
            net.kyori.adventure.text.Component message = plugin.getLanguageManager().get(MessageKey.IPBAN_BROADCAST,
    ...
    protected List<String> tabComplete(CommandSender sender, String[] args) {
        ...
        if (regularArgIndex == 2) {
            return filterCompletions(Arrays.asList("1h", "6h", "12h", "1d", "3d", "7d", "14d", "30d", "1y", "permanent"), currentArg);
        }
